...
* Ingestão por um endpoint de propriedade do publicador para sistemas de análise internos.
...
Para enviar dados para um endpoint de propriedade do publicador,
...
Se você, como publicador, tem controle sobre o arquivo remoto,
...
O passo seguinte é criar o conteúdo JSON que reside no URL remoto.
...
para o valor de account no URL remoto (`"account": "UA-XXXXX-Y"`):
...
O exemplo `amp-analytics` agrega um parâmetro adicional <code>cd1</code>
...
Se o agente do usuário do cliente não oferecer suporte a um método,
o método de precedência mais alta ativado seguinte será usado. 
...
Se o agente do usuário do cliente oferecer suporte ao método `image`,
